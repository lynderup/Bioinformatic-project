...
with tf.name_scope("inputs"):
    tf_inputs = [tf.convert_to_tensor([[i1], [i2]], dtype=tf.float32, name="input%i" % idx) for idx, (i1, i2) in enumerate(it.zip_longest(*inputs, fillvalue=0))]
...
tf.summary.scalar('test', 5)

merged = merged = tf.summary.merge_all()
...
with tf.Session() as session:
    ...
    out, summary = session.run([outputs, merged])
